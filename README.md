# Digital-Literacy-Project

# 🌐 CSE0001: Digital Literacy Project Portfolio
> **Empowering the academic community through responsible and effective digital practices.**

![Course](https://img.shields.io/badge/Course-CSE0001%20Digital%20Literacy-blue?style=for-the-badge&logo=googlechrome&logoColor=white)
![Role](https://img.shields.io/badge/Role-Student%20Digital%20Ambassador-success?style=for-the-badge&logo=microsoftacademic&logoColor=white)
![Institution](https://img.shields.io/badge/Institution-VIT%20Bhopal%20University-8A2BE2?style=for-the-badge&logo=unrealengine&logoColor=white)


## 👤 Ambassador Profile

| Attribute | Details |
| :--- | :--- |
| **Name** | Kanishk Raj |
| **Registration Number** | **25BCE10866** |
| **Degree Program** | First Year B.Tech - All Branches  (CSE Core) |
| **Core Skills** | Python Programming, Digital Problem Solving |

---

## 📌 Project Overview

Welcome to my official **Digital Literacy Portfolio**! I have developed this repository in my capacity as a **Student Digital Ambassador**. The primary objective of this role is to assist peers in effectively navigating the digital landscape—ranging from establishing a professional online presence to practicing safe and responsible digital communication. 

This project encompasses five distinct modules, each addressing a critical component of modern digital literacy.


Google form link: https://docs.google.com/forms/d/1-vs4_FUGVaVi1Tbc5ORzdSDPHsseC1TMKVvogDPkzPU/edit?pli=1

---

## 📂 Repository Architecture

All project files and evidence have been meticulously organized into the following directory structure as required by the assignment guidelines:

```text
📦 digital-literacy-project
 ┣ 📂 report/                   # Contains the comprehensive Project Report (.docx/.pdf) [cite: 2, 14]
 ┣ 📂 task-1-presentation/      # Canva/Prezi design exports and reflections [cite: 14]
 ┣ 📂 task-2-portfolio/         # Screenshots of GitHub, LinkedIn, and Kaggle profiles [cite: 14]
 ┣ 📂 task-3-platforms/         # HackerRank achievements and Google Workspace proof [cite: 14]
 ┣ 📂 task-4-email-etiquette/   # Professional email drafts & Social Media Checklist [cite: 14]
 ┗ 📂 task-5-cybercrime/        # Cybercrime case study & Prevention guidelines [cite: 14]
 ┗ 📜 README.md                 # Project overview, summaries, and vital links [cite: 13]
